	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm"  style="box-sizing: content-box" action= "'+window.location.pathname+'/postImg" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
					html += '		<div class="upload_main"  style="box-sizing: content-box">';
		            ...
		            html += '					<div class="upload_btn">确定</div>';
		            html += '				</div>';
		            html += '			</div>';
					html += '<div id="preview" class="upload_preview" style="box-sizing:content-box"></div>';
					html += '		</div>';
					...
					html += '<form id="uploadForm"  style="box-sizing: content-box" action= "'+window.location.pathname+'/postImg" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
					html += '		<div class="upload_main single_main"  style="box-sizing: content-box">';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            ...
		            html += '				</div>';
		            html += '			</div>';
		            html += '<div id="preview" class="upload_preview"  style="box-sizing: content-box">';
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 8048576) {
						toastr.error('文件"'+ file.name +'"过大！(最大上传1M/文件)');
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					}
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								...
								$(".file_del").click(function() {
									...
									$('#previewImage_'+parseInt($(this).attr("data-index"))).remove();
									console.log('#previewImage_'+parseInt($(this).attr("data-index")));
									return false;	
								});
							}
